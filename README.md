...
ربات تلگرامی با قابلیت انتخاب مدل چت، تشخیص گفتار (Whisper)، تحلیل تصویر، خواندن فایل PDF و فایل‌های متنی، و ساخت تصویر (FLUX)، روی Cloudflare Workers.
...
## مراحل کامل راه‌اندازی
   ...
   6.بعد از اتمام دیپلوی، آدرس Worker خودت رو (چیزی شبیه `https://tel-ai-bot.<account>.workers.dev`) توی مرورگر باز کن و آخرش `/setup` اضافه کن:

```
https://tel-ai-bot.<account>.workers.dev/setup
```
اگه پیام «✅ Webhook ثبت شد» اومد، کارت تمومه و Webhook خودکار ثبت شده.

(روش دستی قبلی هم هنوز کار می‌کنه: `https://api.telegram.org/bot<BOT_TOKEN>/setWebhook?url=<آدرس Worker شما>`)
هنگام کلیک روی دکمه:
...
> ℹ️ پنج متغیر اختیاری دیگه (`TAVILY_API_KEY`, `ALLOWED_USER_IDS`, `CF_ACCOUNT_ID`, `CF_API_TOKEN`, `WEBHOOK_SECRET`) عمداً توی صفحهٔ دیپلوی نیستن — چون دکمهٔ Deploy کلادفلر فعلاً هر فیلدی که ببینه رو الزامی می‌کنه و راهی برای «اختیاری» نشونش دادن نداره. اگه اونا رو هم توی `wrangler.jsonc` می‌ذاشتیم، مجبور می‌شدی همه رو پر کنی. به‌جاش، بعد از دیپلوی اول، هر کدوم رو خواستی از داشبورد اضافه می‌کنی (مرحلهٔ ۷ پایین).
...
7. **(اختیاری) فعال کردن قابلیت‌های بیشتر:** برو به Cloudflare Dashboard → Workers & Pages → Worker خودت → Settings → Variables and Secrets → Add، و هر کدوم از این‌ها رو خواستی اضافه کن (بعد از هر بار افزودن، دکمهٔ Deploy همون‌جا رو بزن تا اعمال بشه):
   ...
   - `WEBHOOK_SECRET` — جلوگیری از درخواست‌های جعلی به ربات. یه رشتهٔ تصادفی بذار (فقط حروف انگلیسی، عدد، `_` و `-`، مثلاً `Kx9_mP2qLw7Rt4Vz`). **بعد از افزودنش و Deploy، حتماً یک‌بار دوباره آدرس `/setup` رو توی مرورگر باز کن**؛ وگرنه ربات جواب نمی‌ده.
...
- **PDF**: متن داخل PDF خونده می‌شه (تا ۱۵ مگابایت و ۲۵ هزار کاراکتر اول). PDFهای اسکن‌شده/عکسی متن ندارن و خونده نمی‌شن.
- **فایل متنی/کد** (txt, html, js, json, py, md و ...): تا ۲۵ هزار کاراکتر اول.
- فایل رو با یک توضیح (کپشن) بفرست، مثلاً «خلاصه کن». بعدش تا ۶ ساعت (یا تا دستور `/new`) می‌تونی دربارهٔ همون فایل سؤال بپرسی. با فرستادن فایل جدید، فایل قبلی جایگزین می‌شه.
